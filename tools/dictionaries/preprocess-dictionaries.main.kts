...
data class DictionaryIndex(
    ...
    val bigrams: Map<String, Map<String, Int>>? = null,
    val trigrams: Map<String, Map<String, Map<String, Int>>>? = null,
    val domainWords: Map<String, List<String>>? = null,
    val commonPhrases: List<PhraseEntry>? = null
)
        ...
        try {
            ...
            fun loadBigrams(file: File): Map<String, Map<String, Int>>? {
                ...
                try {
                    ...
                    jsonObj.keys().forEach { word1 ->
                        val word2Map = jsonObj.getJSONObject(word1)
                        val innerMap = mutableMapOf<String, Int>()
                        word2Map.keys().forEach { word2 ->
                            innerMap[word2] = word2Map.getInt(word2)
                        }
                        result[word1] = innerMap
                    }
                    log("  Loaded bigrams: ${result.size} entries")
                    return result
                } catch (e: Exception) {
                    log("  Warning: Could not load bigrams from ${file.name}: ${e.message}")
                    return null
                }
            }
            
            fun loadTrigrams(file: File): Map<String, Map<String, Map<String, Int>>>? {
                if (!file.exists()) return null
                try {
                    val jsonString = file.readText()
                    val jsonObj = JSONObject(jsonString)
                    val result = mutableMapOf<String, MutableMap<String, MutableMap<String, Int>>>()
                    jsonObj.keys().forEach { word1 ->
                        val word2Obj = jsonObj.getJSONObject(word1)
                        val word2Map = mutableMapOf<String, MutableMap<String, Int>>()
                        word2Obj.keys().forEach { word2 ->
                            val word3Obj = word2Obj.getJSONObject(word2)
                            val word3Map = mutableMapOf<String, Int>()
                            word3Obj.keys().forEach { word3 ->
                                word3Map[word3] = word3Obj.getInt(word3)
                            }
                            word2Map[word2] = word3Map
                        }
                        result[word1] = word2Map
                    }
                    log("  Loaded trigrams: ${result.size} entries")
                    return result
                } catch (e: Exception) {
                    log("  Warning: Could not load trigrams from ${file.name}: ${e.message}")
                    return null
                }
            }
