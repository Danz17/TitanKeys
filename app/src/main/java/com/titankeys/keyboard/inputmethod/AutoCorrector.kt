...
object AutoCorrector {
    ...
    fun loadCorrections(assets: AssetManager, context: Context? = null) {
        try {
            ...
            val standardLocales = listOf("it", "en", "es", "fr", "de", "pl", "x-titankeys")

            for (locale in standardLocales) {
                try {
                    // First load custom corrections (if they exist)
                    if (context != null) {
                        val customCorrections = com.titankeys.keyboard.SettingsManager.getCustomAutoCorrections(context, locale)
            ...
            if (context != null) {
                try {
                    val prefs = com.titankeys.keyboard.SettingsManager.getPreferences(context)
                    ...
                    for ((key, value) in allPrefs) {
                        if (key.startsWith("auto_correct_custom_") && value is String) {
                            ...
                            if (languageCode !in standardLocales) {
                                try {
                                    val customCorrections = com.titankeys.keyboard.SettingsManager.getCustomAutoCorrections(context, languageCode)
                                    if (customCorrections.isNotEmpty()) {
                                        val customJson = correctionsToJson(customCorrections)
    ...
    fun getCorrection(word: String, locale: String? = null, context: Context? = null): String? {
        ...
        if (context != null) {
            val enabledLanguages = com.titankeys.keyboard.SettingsManager.getAutoCorrectEnabledLanguages(context)
    ...
    fun processText(
        ...
        val enabledLanguages = if (context != null) {
            com.titankeys.keyboard.SettingsManager.getAutoCorrectEnabledLanguages(context)
        } else {
            emptySet<String>()
        }
        ...
        // Build candidate languages: IME language + x-titankeys (if available)
        val candidates = buildSet {
            if (corrections.containsKey(imeLanguage)) add(imeLanguage)
            if (corrections.containsKey("x-titankeys")) add("x-titankeys")
        }

        // Apply toggle filter (if not empty). x-titankeys remains only if enabled.
