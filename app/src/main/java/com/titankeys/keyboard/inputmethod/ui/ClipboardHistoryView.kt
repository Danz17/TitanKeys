...
import com.titankeys.keyboard.clipboard.ClipboardHistoryEntry
 ...
 * Supports both text and image clipboard entries.
 */
class ClipboardHistoryView(
    ...
    private fun onEntryClicked(entry: ClipboardHistoryEntry) {
        if (entry.isImage) {
            // For images, set to system clipboard and show toast
            entry.imagePath?.let { path ->
                clipboardHistoryManager.pasteImage(path)
                Toast.makeText(context, R.string.clipboard_image_copied, Toast.LENGTH_SHORT).show()
            }
        } else {
            // For text, commit directly to input connection
            entry.text?.let { text ->
    ...
    private inner class ClipboardHistoryAdapter :
        ...
        override fun getItemViewType(position: Int): Int {
            ...
            return if (entry.isImage) Companion.VIEW_TYPE_IMAGE else Companion.VIEW_TYPE_TEXT
        }
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ClipboardHistoryViewHolder {
            ...
            return if (viewType == Companion.VIEW_TYPE_IMAGE) {
                ...
                ClipboardHistoryViewHolder(container, null, imageView)
            } else {
                ...
                ClipboardHistoryViewHolder(container, textView, null)
            }
        }
        ...
        override fun onBindViewHolder(holder: ClipboardHistoryViewHolder, position: Int) {
            ...
            if (entry.isImage && holder.imageView != null) {
                // Load image thumbnail
                entry.imagePath?.let { path ->
                    try {
                        val file = File(path)
                        if (file.exists()) {
                            // Load scaled down bitmap to save memory
            ...
            } else if (holder.textView != null) {
                holder.textView.text = entry.displayText
            }
