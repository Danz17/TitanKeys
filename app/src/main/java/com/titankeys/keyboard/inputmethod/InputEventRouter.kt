...
import com.titankeys.keyboard.inputmethod.AltSymManager
import com.titankeys.keyboard.core.SymLayoutController
import com.titankeys.keyboard.core.SymLayoutController.SymKeyResult
...
import com.titankeys.keyboard.core.AutoSpaceTracker
...
import com.titankeys.keyboard.data.layout.isRealMultiTap
...
class InputEventRouter(
    ...
    var suggestionController: com.titankeys.keyboard.core.suggestions.SuggestionController? = null
    ...
    private fun commitTextWithTracking(ic: InputConnection?, text: CharSequence, trackWord: Boolean = true) {
        Log.d("TitanKeysIME", "commitTextWithTracking enter: '$text', trackWord=$trackWord")
        onCommitText?.invoke()
        ic?.commitText(text, 1)
        if (trackWord) {
            Log.d("TitanKeysIME", "commitTextWithTracking notify SC: '$text'")
    ...
    fun routeEditableFieldKeyDown(
        ...
        if (isAlphabeticKey && LayoutMappingRepository.isMapped(keyCode)) {
            ...
            if (char.isNotEmpty() && char[0].isLetter()) {
                Log.d("TitanKeysIME", "layout commit: '$char'")
                commitTextWithTracking(ic, char)
                Handler(Looper.getMainLooper()).postDelayed({
                    callbacks.updateStatusBar()
                }, params.cursorUpdateDelayMs)
        ...
        if (ic != null && event != null && event.unicodeChar != 0) {
            ...
            if (ch.isLetter()) {
                Log.d("TitanKeysIME", "fallback commit: '$ch'")
    ...
    fun handleTextInputPipeline(
        ...
        inputContextState: com.titankeys.keyboard.core.InputContextState?,
        ...
        val boundarySet = com.titankeys.keyboard.core.Punctuation.BOUNDARY
        ...
        if (inputContextState != null && enableShiftOneShot != null) {
            val shouldCap = com.titankeys.keyboard.inputmethod.AutoCapitalizeHelper.shouldCapitalizeAfterBoundary(
                context = context,
        ...
        if (isEnterKey) {
            ...
            var replaceResult: com.titankeys.keyboard.core.suggestions.AutoReplaceController.ReplaceResult? = null
