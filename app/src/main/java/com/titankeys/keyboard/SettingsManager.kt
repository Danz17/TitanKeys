...
object SettingsManager {
    ...
    private const val PREFS_NAME = "titankeys_prefs"
    ...
    private const val KEY_NEXT_WORD_PREDICTION_ENABLED = "next_word_prediction_enabled"
    private const val KEY_NEXT_WORD_PREDICTION_MODE = "next_word_prediction_mode"  // 0=CURRENT_WORD, 1=NEXT_WORD, 2=HYBRID
    private const val KEY_USER_LEARNING_ENABLED = "user_learning_enabled"
    private const val KEY_SUGGESTION_FADE_DELAY_MS = "suggestion_fade_delay_ms"

    private const val DEFAULT_USE_KEYBOARD_PROXIMITY = false
    private const val DEFAULT_USE_EDIT_TYPE_RANKING = false
    private const val DEFAULT_NEXT_WORD_PREDICTION_ENABLED = true
    private const val DEFAULT_NEXT_WORD_PREDICTION_MODE = 1  // NEXT_WORD
    private const val DEFAULT_USER_LEARNING_ENABLED = true
    private const val DEFAULT_SUGGESTION_FADE_DELAY_MS = 3000L  // 3 seconds before suggestions fade to show cursor scroll
     ...
     */
    fun getNextWordPredictionEnabled(context: Context): Boolean {
        return getPreferences(context).getBoolean(KEY_NEXT_WORD_PREDICTION_ENABLED, DEFAULT_NEXT_WORD_PREDICTION_ENABLED)
    }

    /**
     * Enables or disables next-word prediction.
     */
    fun setNextWordPredictionEnabled(context: Context, enabled: Boolean) {
        getPreferences(context).edit()
            .putBoolean(KEY_NEXT_WORD_PREDICTION_ENABLED, enabled)
            .apply()
    }

    /**
     * Returns the next-word prediction mode (0=CURRENT_WORD, 1=NEXT_WORD, 2=HYBRID).
     */
    fun getNextWordPredictionMode(context: Context): Int {
        return getPreferences(context).getInt(KEY_NEXT_WORD_PREDICTION_MODE, DEFAULT_NEXT_WORD_PREDICTION_MODE)
    }

    /**
     * Sets the next-word prediction mode (0=CURRENT_WORD, 1=NEXT_WORD, 2=HYBRID).
     */
    fun setNextWordPredictionMode(context: Context, mode: Int) {
        getPreferences(context).edit()
            .putInt(KEY_NEXT_WORD_PREDICTION_MODE, mode.coerceIn(0, 2))
            .apply()
    }

    /**
     * Returns whether user learning is enabled.
     */
    fun getUserLearningEnabled(context: Context): Boolean {
        return getPreferences(context).getBoolean(KEY_USER_LEARNING_ENABLED, DEFAULT_USER_LEARNING_ENABLED)
    }

    /**
     * Enables or disables user learning.
     */
    fun setUserLearningEnabled(context: Context, enabled: Boolean) {
        getPreferences(context).edit()
            .putBoolean(KEY_USER_LEARNING_ENABLED, enabled)
            .apply()
    }

    /**
     * Gets the delay in milliseconds before suggestions fade out to show cursor scroll bar.
     * Default is 3000ms (3 seconds).
     */
    fun getSuggestionFadeDelayMs(context: Context): Long {
        return getPreferences(context).getLong(KEY_SUGGESTION_FADE_DELAY_MS, DEFAULT_SUGGESTION_FADE_DELAY_MS)
    }

    /**
     * Sets the delay in milliseconds before suggestions fade out to show cursor scroll bar.
     */
    fun setSuggestionFadeDelayMs(context: Context, delayMs: Long) {
        getPreferences(context).edit()
            .putLong(KEY_SUGGESTION_FADE_DELAY_MS, delayMs)
            .apply()
    }
    ...
    fun getAutoCorrectEnabledLanguages(context: Context): Set<String> {
        ...
        // Default: system language + x-titankeys, with fallback to English
        ...
        return setOf(defaultLanguage, "x-titankeys")
    }
     ...
     */
    fun saveNavModeKeyMappings(context: Context, mappings: Map<Int, com.titankeys.keyboard.data.mappings.KeyMappingLoader.CtrlMapping>) {
        try {
            val keyCodeToName = mapOf(
                KeyEvent.KEYCODE_Q to "KEYCODE_Q", KeyEvent.KEYCODE_W to "KEYCODE_W",
                KeyEvent.KEYCODE_E to "KEYCODE_E", KeyEvent.KEYCODE_R to "KEYCODE_R",
    ...
    private fun isLayoutAvailable(context: Context, layoutName: String): Boolean {
        if (com.titankeys.keyboard.data.layout.LayoutFileStore.layoutExists(context, layoutName)) {
            return true
        }
        return try {
