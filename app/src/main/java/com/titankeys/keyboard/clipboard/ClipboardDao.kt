...
class ClipboardDao private constructor(
    ...
    private val cache = mutableListOf<ClipboardHistoryEntry>().apply {
        db.readableDatabase.query(
            TABLE,
            arrayOf(COLUMN_ID, COLUMN_TIMESTAMP, COLUMN_PINNED, COLUMN_TEXT, COLUMN_IMAGE_PATH),
            null,
            null,
            null,
            null,
    ...
    /**
     * Add a new text clipboard entry or update existing one with same text.
    ...
    fun addClip(timestamp: Long, pinned: Boolean, text: String, retentionMinutes: Long? = null) {
        ...
        val existingIndex = cache.indexOfFirst { it.text == text && it.imagePath == null }
        if (existingIndex >= 0) {
        ...
        // Insert new entry
        insertNewEntry(timestamp, pinned, text, null)
    }

    /**
     * Add a new image clipboard entry.
     * @param imagePath Path to the saved image file
     * @param retentionMinutes Optional retention time in minutes.
     */
    fun addImageClip(timestamp: Long, pinned: Boolean, imagePath: String, retentionMinutes: Long? = null) {
        if (retentionMinutes != null) {
            clearOldClips(now = true, retentionMinutes = retentionMinutes)
        } else {
            clearOldClips()
        ...
        // Don't deduplicate images - each screenshot is unique
        insertNewEntry(timestamp, pinned, null, imagePath)
    }

    private fun insertNewEntry(timestamp: Long, pinned: Boolean, text: String?, imagePath: String?) {
        ...
        val entry = ClipboardHistoryEntry(rowId, timestamp, pinned, text, imagePath)
        cache.add(entry)
        cache.sort()
    ...
    fun deleteClipAt(index: Int) {
        ...
        entry.imagePath?.let { path ->
            try {
                ...
                Log.w(TAG, "Failed to delete image file: $path", e)
            }
        }
    ...
    fun clearOldClips(now: Boolean = false, retentionMinutes: Long = 120) {
        ...
        // Delete image files for entries being removed
        ...
        val indicesToRemove = entriesToRemove.map { cache.indexOf(it) }
    ...
    companion object {
        ...
        fun getInstance(context: Context): ClipboardDao? {
            return instance ?: synchronized(this) {
                instance ?: try {
                    ClipboardDao(context, ClipboardDatabase.getInstance(context)).also {
                        instance = it
                    }
                } catch (e: Throwable) {
